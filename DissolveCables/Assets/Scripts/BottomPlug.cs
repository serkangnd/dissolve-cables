    ...
    */
    ...
    private void Update()
    {
        if (isSelected)
        {
            ...
            transform.position = Vector3.Lerp(transform.position, movementPosition.transform.position, .3f);
        ...
        {
            //After selection our game object will automatically go to movementPosition
            transform.position = Vector3.Lerp(transform.position, targetSocket.transform.position, .3f);
            //If the distance between our plug and the position we send is less than .10f, the if block here will work.
